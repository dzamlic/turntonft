...
import { Authorized, Connection } from '@solana/web3.js';
...
const TRACKING_ID = "G-XT905T58R3";
...
export const ArtCreateView = () => {
  ...
  const [errorConnected, setErrorConnected] = useState<boolean>(false);
  ...
  const gotoStep = useCallback(
    (_step: number) => {
      ...
      if (_step === 0 && init) setStepsVisible(true);
    },
    [history],
  );
  ...
  }, [stepsVisible, init, wallet.connected]);
  ...
  return (
    <>
      ...
      <Row>
      <Button onClick={ (showSteps)}>Create nft</Button>
      </Row>    
      <br></br>
      <Row>{ errorConnected && !wallet.connected && (
        <p className="red-p">
          You need to connect your wallet.
        </p>
      )}</Row>
      <br></br>
      <Row>{ !init && (<img style={{  width: '100%', height: 'auto'  }} src="/nft-image.jpg" alt="convert to nft, turn to nft"></img>)}</Row>
      ...
      <Row><span>power by Metaplex</span> </Row>
    </>
  );
};
